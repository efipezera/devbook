...
//BuscarUsuarios busca os usuários do banco de dados.
...
//BuscarUsuario busca um usuário específico do banco de dados.
...
func AtualizarUsuario(rw http.ResponseWriter, r *http.Request) {
	...
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(rw, http.StatusForbidden, errors.New("não é possível atualizar um usuário que não seja o seu"))
		return
	}
	corpoRequisicao, erro := ioutil.ReadAll(r.Body)
	if erro != nil {
...
//DeletarUsuario exclue um usuário do banco de dados.
func DeletarUsuario(rw http.ResponseWriter, r *http.Request) {
	...
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(rw, http.StatusForbidden, errors.New("não é possível deletar um usuário que não seja o seu"))
		return
	}
	db, erro := banco.Conectar()
	if erro != nil {
...
func AtualizarSenha(rw http.ResponseWriter, r *http.Request) {
	...
	if usuarioIDNoToken != usuarioID {
		respostas.Erro(rw, http.StatusForbidden, errors.New("não é possível atualizar a senha de um usuário que não seja o seu"))
		return
	}
	corpoRequisicao, erro := ioutil.ReadAll(r.Body)
	...
	if erro = seguranca.VerificarSenha(senhaSalvaNoBanco, senha.Atual); erro != nil {
		respostas.Erro(rw, http.StatusUnauthorized, errors.New("a senha atual não condiz com a senha que está salva no banco"))
		return
	}
